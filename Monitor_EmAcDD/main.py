...
class DriverMonitor:
    ...
    def run(self):
        ...
        retry_count = 0
        max_retries = 3 if not is_video_file else 0  # Allow retries for cameras, not for videos

        while True:
            ok, img = self.cap.read()
            if not ok:
                if is_video_file:
                    print("[info] Video ended. Exiting...", flush=True)
                    break
                else:
                    retry_count += 1
                    if retry_count > max_retries:
                        print("[error] Failed to grab frame after multiple retries. Exiting...", flush=True)
                        break
                    print(f"[warn] Failed to grab frame; retrying ({retry_count}/{max_retries})...", flush=True)
                    time.sleep(0.05)
                    continue
            ...
            if faces:
                ...
                self.stats.update(
                    ...
                    blink_total=self.eyes.blinkCounter,
            ...
            cv2.waitKey(frame_delay)  # Instead of waitKey(1)
...
# python3 -m Monitor_EmAcDD.main --cam 1 
# python3 -m Monitor_EmAcDD.main --url '/Users/amara/SideProjects/Research/Face-detection/testvideo.mov'